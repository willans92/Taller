...
class EMPRESA {
	...
	function contructor($id_empresa, $nombre, $razon_social, $logo, $aniversario, $fecha_afiliacion, $empresacol, $nit, $direccion, $nro_factura, $fecha_factura, $llave_dosificacion, $nro_autorizacion){
	...
	function cargar($resultado){
		if ($resultado->num_rows > 0) {
			...
			while($row = $resultado->fetch_assoc()) {
				...
				$empresa->empresacol=$row['empresacol']==null?"":$row['empresacol'];
...
return $empresa[0];
	...
	function modificar($id_empresa){
		$consulta="update taller.EMPRESA set id_empresa =".$this->id_empresa.", nombre ='".$this->nombre."', razon_social ='".$this->razon_social."', logo ='".$this->logo."', aniversario ='".$this->aniversario."', fecha_afiliacion ='".$this->fecha_afiliacion."', empresacol ='".$this->empresacol."', nit ='".$this->nit."', direccion ='".$this->direccion."', nro_factura ='".$this->nro_factura."', fecha_factura ='".$this->fecha_factura."', llave_dosificacion ='".$this->llave_dosificacion."', nro_autorizacion ='".$this->nro_autorizacion."' where id_empresa=".$id_empresa;
		$result=$this->CON->consulta($consulta);
		return $ret['cant'];
	}

	function insertar(){
		$consulta="insert into taller.EMPRESA(id_empresa, nombre, razon_social, logo, aniversario, fecha_afiliacion, empresacol, nit, direccion, nro_factura, fecha_factura, llave_dosificacion, nro_autorizacion) values(".$this->id_empresa.",'".$this->nombre."','".$this->razon_social."','".$this->logo."','".$this->aniversario."','".$this->fecha_afiliacion."','".$this->empresacol."','".$this->nit."','".$this->direccion."','".$this->nro_factura."','".$this->fecha_factura."','".$this->llave_dosificacion."','".$this->nro_autorizacion."')";
