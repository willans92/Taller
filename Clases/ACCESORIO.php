...
class ACCESORIO {
    ...
    function buscarXEmpresa($id) {
        $consulta = "select * from taller.ACCESORIO where id_empresa=$id";
        $result = $this->CON->consulta($consulta);
        $empresa = $this->rellenar($result);
        if ($empresa == null) {
            return null;
    ...
    function insertar() {
        $consulta = "insert into taller.ACCESORIO(descripcion, id_empresa) values('" . $this->descripcion . "'," . $this->id_empresa . ")";
