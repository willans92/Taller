...
class CLIENTE {
    ...
    function buscarTexto($texto,$empresa) {
        $consulta = "SELECT *
                    FROM taller.cliente
                    where cliente.id_empresa=$empresa and cliente.id_cliente in (SELECT auto.id_cliente FROM taller.auto where auto.placa like '%$texto%' or auto.nro_chasis like '%$texto%')
                            or cliente.id_cliente in (SELECT cliente.id_cliente FROM taller.cliente where cliente.id_empresa=$empresa and cliente.nombre like '%$texto%' or cliente.ci like '%$texto%')
                        or cliente.id_cliente in (SELECT auto.id_cliente FROM taller.auto, taller.reparacion where reparacion.ot like '%$texto%' and reparacion.id_auto=auto.id_auto)";
        $result = $this->CON->consulta($consulta);
        $listado = $this->rellenar($result);
        return $listado;
    }
